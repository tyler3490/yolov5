...
#res = [(mu,P,mu)]
res=[]
N = 15 #to take an initial stretch and see what happens if you later lose the observation
...
while(True):
    ...
    if(camshift):
        ...
        ##calculate center of the "roibix"##
        if(yo<error or bgs.sum()<50 ):
            mu,P,pred= KalmanFilter(mu,P,F,Q,B,a,None,H,R)
            m="None"
            mm=False

        else:
            mu,P,pred= KalmanFilter(mu,P,F,Q,B,a,np.array([xo,yo]),H,R)
            m="normal"
            mm=True
        ...
        for _ in range(fps*2):
            mu2,P2,pred2= KalmanFilter(mu2,P2,F,Q,B,a,None,H,R)
            res2 += [(mu2,P2)]
        ...
        for n in range(len(listCenterX)): #center of the roibox
